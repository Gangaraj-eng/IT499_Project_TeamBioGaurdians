...
def create_Data_Record_Dynamic1(outfileName, img_addrs, audio_addrs, video_addrs, labels):
    ...
    for i in range(len(audio_addrs)):
       ...
       video_frames = frames_from_video_file(video_addrs[i],n_frames=10, output_size=img_dimensions)
...
def create_Data_Record_Dynamic2(outfileName, img1_addrs, img2_addrs, video_addrs1, video_addrs2, audio_addrs, labels):
    ...
    for i in range(len(audio_addrs)):
         ...
         audio = extract_audio_feature(os.path.join(rootAudioPath, audio_addrs[i]), 'mfcc', n_mfcc=20)
         audio = np.array(audio, dtype=np.float64)
         video_frames1 = frames_from_video_file(video_addrs1[i],n_frames=10, output_size=img_dimensions)
         video_frames2 = frames_from_video_file(video_addrs2[i],n_frames=10, output_size=img_dimensions)
